...
def load_data(dataset, hist_len, target_ctr, use_neg_hist, logarithm_input, standardize_input, emb_dim, exclude, combiner):
    if target_ctr > 0:
        ctr_str = f"_{target_ctr}"
    else:
        ctr_str = ""

    processed_file = f"./dataset/{dataset}/processed_input_{hist_len}{ctr_str}"
    if dataset == "movielens-20":
    ...
    if path.exists(processed_file):
        ...
        if use_neg_hist:
            fname += "_neg"
        fpath = f"./dataset/{dataset}/{fname}.pkl"
        if not path.exists(fpath):
            print("FB-DLRM-compatible input generation")
            if dataset == "movielens-20":
                getMovieLensData("./dataset/movielens-20/", fpath, clk_thres=clk_thres)
            elif dataset == "taobao":
                getTaobaoAdsData("./dataset/taobao/", fpath, use_neg=use_neg_hist)
            else:
    ...
    if len(exclude) > 0:
        ...
        feature_columns = list(filter(lambda x: x.name not in exclude, feature_columns))
    print(feature_columns)
