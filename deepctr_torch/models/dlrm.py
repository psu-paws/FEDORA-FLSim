...
class DLRM(BaseModel):
    ...
    def forward(self, X):
        ...
        # Feature interaction
        #z = self.interact_features(nn.functional.layer_norm(bot_x, bot_x.shape),
        #                            [nn.functional.layer_norm(t.reshape([-1, t.shape[-1]]), t.reshape([-1, t.shape[-1]]).shape) for t in sparse_embedding_list])
