...
def run_centralized_training(args):
    x_train, y_train, x_test, y_test, behavior_feature_list, feature_columns = load_data(args.dataset, args.hist_len, args.target_ctr, args.model == "dien", args.logarithm_input, args.standardize_input, args.emb_dim, args.exclude, args.combiner)
...
def run_fl_training(args):
    ...
    test_data = {
            "x_test": {k: np.concatenate([c.x_test[k] for c in test_clients], axis=0) for k in features},
            "y_test": np.concatenate([c.y_test for c in test_clients], axis=0)}
...
def run_fl_per_rank(args, rank, world_size, train_clients, test_data, total_train_cids, feature_columns, behavior_feature_list):
    ...
    # Generate server
    if len(args.dp_oram_params) > 0:
        dp_oram_params = args.dp_oram_params.split(",")
    else:
        dp_oram_params = []
    server = Server(rank, world_size, train_clients, test_data, model, device, args.server_optimizer, args.server_lr, total_train_cids, args.metrics, args.seed, args.staleness, dp_oram_params, args.dp_oram_eps, args.dp_max_feats)
